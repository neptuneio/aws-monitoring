...
###############################################
# Purpose : This script fetches a process monitoring script, updates access/secret keys and other variables, and pushes the monitoring script into crontab
...
if [ -z "$PROCESS_NAME" ] ; then
  echo -e "${red}You didn't specify a process name${NC}\n"
  echo -e "Please give a proper value to variable PROCESS_NAME and rerun the above command"
  echo -e "Exiting"
  exit 1;
fi

# Fetch the monitoring script
curl -s -O https://s3.amazonaws.com/downloads/aws-monitoring-scripts/processStatus.sh

MONITORING_FILE=${PROCESS_NAME}_status.sh
mv processStatus.sh $MONITORING_FILE

# Replace appropriate varibales one-time in the monitoring script
sed -i "s/NEPTUNEIO_PROCESS_NAME/$PROCESS_NAME/" $MONITORING_FILE
