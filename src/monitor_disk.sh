...
###############################################
# Purpose : This script fetches a Disk monitoring script, updates access/secret keys and other variables, and pushes the monitoring script into crontab
...
# Check if user gave a non-empty PROCESS_NAME

if [ -z "$DISK_MOUNT" ] ; then
  echo -e "${red}You didn't specify a disk mount.${NC}\n"
  echo -e "Please give a proper value to variable DISK_MOUNT (for e.g : /dev/xvda1 ) and rerun the above command"
  echo -e "Exiting"
  exit 1;
fi

# Fetch the monitoring script
curl -s -O https://s3.amazonaws.com/downloads/aws-monitoring-scripts/diskStatus.sh

CLEANED_UP_DISK_MOUNT=`echo $DISK_MOUNT | sed 's/^\///' | sed 's/\//_/g'`
MONITORING_FILE=${CLEANED_UP_DISK_MOUNT}_utlization_status.sh
mv diskStatus.sh $MONITORING_FILE

# Replace appropriate varibales one-time in the monitoring script
sed -i "s#NEPTUNEIO_DISK_MOUNT#$DISK_MOUNT#" $MONITORING_FILE
sed -i "s/NEPTUNEIO_CLEANED_UP_DISK_MOUNT/$CLEANED_UP_DISK_MOUNT/" $MONITORING_FILE
